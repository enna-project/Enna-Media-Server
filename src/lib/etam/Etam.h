...
extern "C" {
  ...
  typedef Eina_Bool (*Etam_DB_Validate_Cb)(void *data, Etam_DB *db, Etam_DB_Op op, const char *name, int index);
  typedef Eina_Bool (*Etam_Coll_Validate_Cb)(void *data, Etam_Collection *coll, Etam_Collection_Op op, const char *key, const char *name, Eina_Value *value);
  ...
  EAPI Etam_Collection *etam_collection_open(Etam_DB *db,
                                             const char *name,
...
#define ETAM_REQUEST_LONG_LONG(Type, Name, Value) Type, ETAM_T_LONG_LONG, ETAM_TYPECHECK(const char *, Name), ETAM_TYPECHECK(long long, Value)
  ...
  EAPI Etam_Process *etam_request_entry_value_get(Etam_Request *coll, const Eina_Value_Struct_Desc *desc,
                                                  Etam_Entry_Value_Cb value_cb, Etam_Entry_Error_Cb error_cb, void *user_data,
                                                  const char *key);
  EAPI Etam_Process *etam_request_entry_data_get(Etam_Collection *coll, const Etam_Data_Descriptor *desc,
                                                 Etam_Entry_Data_Cb value_cb, Etam_Entry_Error_Cb error_cb, void *user_data,
                                                 const char *key);

  EAPI Etam_Process *etam_request_entries_value_get(Etam_Collection *coll, const Eina_Value_Struct_Desc *desc,
                                                    Etam_Entry_Value_Cb value_cb, Etam_Entry_Error_Cb error_cb, void *user_data,
                                                    ...);
  EAPI Etam_Process *etam_request_entries_data_get(Etam_Collection *coll, const Etam_Data_Descriptor *desc,
                                                   Etam_Entry_Value_Cb value_cb, Etam_Entry_Error_Cb error_cb, void *user_data,
                                                   ...);

  EAPI void etam_request_cancel(Etam_Request *rq);
  ...
  EAPI void etam_collection_entry_value_push(Etam_Collection *coll, const char *key, const char *name, Eina_Value *value);
  EAPI void etam_collection_entry_data_push(Etam_Collection *coll, const char *key, const char *name, Etam_Data_Type type, void *data, int count);
